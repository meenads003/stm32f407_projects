...
uint32_t receivedValue;// Variable to store received notification value
/* USER CODE END PV */
...
int main(void)
{
  ...
  myTask02Handle = osThreadNew(SenderTask, NULL, &myTask02_attributes);

  /* creation of myTask03 */
  myTask03Handle = osThreadNew(ReceiverTask, NULL, &myTask03_attributes);
...
void SenderTask(void *argument)
{
  /* USER CODE BEGIN SenderTask */
	//Value to send through notification
	  uint32_t value = 1;
  /* Infinite loop */
  for(;;)
  {
	  // Send notification to ReceiverTask
	      xTaskNotify((TaskHandle_t)myTask03Handle,
	                  value,
...
void ReceiverTask(void *argument)
{
  ...
  for(;;)
  {
	  //Wait until notification is received
	      if(xTaskNotifyWait(0,                 // Don't clear bits on entry
	                         0,                 // Don't clear bits on exit
	                         &receivedValue,    // Store received value
	                         portMAX_DELAY)     // Wait forever
	                         == pdTRUE)
	      {
	        // Toggle Orange LED when notification arrives
